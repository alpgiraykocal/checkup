...
const GLOSSARY = [
  ...
  { k: 'Gerçek faydalanıcı (UBO)', kEn: 'Beneficial owner (UBO)', tr: 'Tüzel kişiyi nihai olarak sahiplenen veya kontrol eden gerçek kişi. Sahiplik eşiği ya da kontrol testiyle bulunur.', en: 'The natural person who ultimately owns or controls a legal entity, found via an ownership threshold or a control test.' },
  ...
  { k: 'BTL / ATL testi', kEn: 'BTL / ATL testing', tr: 'Eşik altı (below-the-line) testi eşiğin hemen altında kalan işlemlerde kaçırılan şüphe olup olmadığını arar; eşik üstü (above-the-line) analizi ise gereksiz alert üretimini ölçer.', en: 'Below-the-line testing looks for missed suspicion just under the threshold; above-the-line analysis measures excessive alert generation.' },
  { k: 'Tuning / kalibrasyon', kEn: 'Tuning / calibration', tr: 'Senaryo eşik ve parametrelerinin veriye bakılarak yeniden ayarlanması.', en: 'Re-setting scenario thresholds and parameters based on data.' },
  ...
  { k: 'Nested ilişki', kEn: 'Nested relationship', tr: 'Muhabir bankanın kendi müşterisi olan başka bir bankaya, sizin hesabınız üzerinden dolaylı erişim sağlaması. Görmediğiniz bir müşteri kitlesi yaratır.', en: 'A respondent bank giving its own bank customers indirect access through your account, creating a customer base you cannot see.' },
  { k: 'Payable-through hesap', kEn: 'Payable-through account', tr: 'Muhabir bankanın müşterilerinin doğrudan işlem yapabildiği hesap; muhabirin müşterisi fiilen sizin sisteminizi kullanır.', en: 'An account through which a respondent\'s customers transact directly, effectively using your systems.' },
  { k: '%50 kuralı', kEn: '50% rule', tr: 'Yaptırım listesindeki kişilerin doğrudan veya dolaylı olarak %50 ve üzerinde sahip olduğu şirketlerin de listelenmiş sayılması.', en: 'Entities owned 50% or more, directly or indirectly, by listed persons are themselves treated as listed.' },
  { k: 'Transliterasyon', kEn: 'Transliteration', tr: 'Bir ismin farklı alfabe ve yazımlarla aktarılması. Tarama motorunun bunları eşleştirebilmesi gerekir.', en: 'Rendering a name across different scripts and spellings; the screening engine must match across them.' },
  ...
  { k: 'Dual-use (çift kullanımlı)', kEn: 'Dual-use', tr: 'Hem sivil hem askeri amaçla kullanılabilen, ihracat kontrol rejimlerine tabi mal ve teknolojiler.', en: 'Goods and technologies usable for both civilian and military purposes, subject to export control regimes.' },
  ...
  { k: 'Unhosted cüzdan', kEn: 'Unhosted wallet', tr: 'Bir hizmet sağlayıcı tarafından barındırılmayan, anahtarları doğrudan kullanıcıda olan kripto cüzdanı.', en: 'A crypto wallet not held by a service provider, with keys directly under the user\'s control.' },
  ...
  { k: 'KAGK / NPO', kEn: 'NPO', tr: 'Kâr amacı gütmeyen kuruluş. Terör finansmanı açısından ayrı bir risk kategorisi olarak ele alınması beklenir.', en: 'Non-profit organisation, expected to be treated as a distinct risk category for terrorist financing purposes.' },
  { k: 'Ayna işlem (mirror trade)', kEn: 'Mirror trade', tr: 'Aynı varlığın farklı ülkelerde eşleşen alım-satımıyla değerin sınır ötesine taşınması.', en: 'Moving value across borders through matched buy and sell trades of the same asset in different countries.' },
  ...
  { k: 'Kayıt dışı değer transferi', kEn: 'Informal value transfer (IVTS)', tr: 'Bankacılık sistemi dışında, güvene dayalı mahsuplaşmayla çalışan para aktarım ağları.', en: 'Value transfer networks operating outside the banking system through trust-based settlement.' }
];
